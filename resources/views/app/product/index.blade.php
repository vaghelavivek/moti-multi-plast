            ...
            <ul class="flex flex-wrap text-sm font-medium text-center border-b text-gray-500 sm:px-0 px-4">
                ...
                <li>
                    <a href="{{ route('category') }}"
                        class="inline-block p-4 rounded-t-lg hover:text-gray-600 hover:bg-gray-50">Category</a>
                </li>
            </ul>
                                    ...
                                    <tbody id="table-body">
                                        {{-- @for ($i = 0; $i < 10; $i++)
                                            <tr class="border-b  hover:bg-gray-100 ">
                                                ...
                                                <td class="px-4 py-2">₹20 per piece</td>
                                                <td
...
<script>
        ...
        if (arrObj.data.length) {
            arrObj.data.map((row, index) => {
                insertStr += coreTableRow(
                    row.media ?
                    `<img src="{{ Request::root() . '/storage/' }}${row.media}" alt="img" class="w-auto h-8 mr-3">` :
    ...
    fetchProductData("{{ route('api.product.get') }}")

    searchInput.addEventListener('input', debounce(
        (e) => {
            fetchProductData(`{{ Request::root() }}/product/get?keyword=${e.target.value}`);
        }, 800
    ))
